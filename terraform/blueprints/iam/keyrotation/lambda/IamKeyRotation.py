...
try:
    config = lambdac.get_function_configuration(FunctionName='IamKeyRotation')
    key_max_age = int(config['Environment']['Variables']['KeyMaxAge'])
...
def get_user_ids(event):
    ...
    if 'Group' in event:
        ...
        return list(map(lambda i: i['UserId'], group['Users']))
