...
resource "aws_iam_role" "awstats_cloudformation" {
  name = "bedrock-awstats-cloudformation"
  assume_role_policy = "${data.aws_iam_policy_document.cloudformation_assume_role_policy.json}"
}

resource "aws_iam_role_policy_attachment" "ec2_full_access" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEC2FullAccess"
  role = "${aws_iam_role.awstats_cloudformation.name}"
}
...
resource "aws_iam_role_policy_attachment" "cloudformation_iam_passrole" {
  ...
  role = "${aws_iam_role.awstats_cloudformation.id}"
}
