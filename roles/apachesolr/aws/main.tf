 ...
 *
 * * Access for managing Solr hosts specific to this blueprint
 */
...
resource "aws_iam_role" "solr_admin" {
  name               = "bedrock-apachesolr-admin"
  assume_role_policy = "${data.aws_iam_policy_document.assume_role_policy.json}"
}

resource "aws_iam_role_policy_attachment" "ec2_readonly" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEC2ReadOnlyAccess"
  role       = "${aws_iam_role.solr_admin.name}"
}
...
resource "aws_iam_role_policy_attachment" "iam_passrole" {
  ...
  role       = "${aws_iam_role.solr_admin.id}"
}
